...
class BartTokenizer(PreTrainedTokenizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """We specifically add this part for efficiently seaching the added tokens in the original vocab."""
        #change begins
        self.a_vocab = {k:v for k,v in self.encoder.items() if v >= 50265}
        for i in 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789':
            self.a_vocab[i] = len(self.a_vocab)
        self.a_vocab['Ġ'] = len(self.a_vocab)
        #change ends
        ...
        for l in range(len(w), 0, -1):
            for i in range(0, len(w) - l + 1):
                if w[i] == '-':
                    continue
                subword = w[i:i + l]
                if subword in self.a_vocab: return subword, w[:i] + l * '-' + w[i + l:], i
